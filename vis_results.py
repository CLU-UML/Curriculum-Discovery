...
def name(dataset, curr):
    ...
    # d = dataset[0]
    c = curr[0]
    b = 'B' if 'balanced' in dataset or 'chaos' in dataset else 'F'
...
def curr_names(row):
    ...
    if row['curr'] == 'loss':
        ...
        # row['curr'] = 'SPL'
        row['curr'] = 'Self-paced Learning' if row['spl_mode'] == 'easy'\
    ...
    lng_names = {'W': 'SL-CL', 'to_SbL1W_C': 'WR-CL'}
    if row['curr'] in ['sampling', 'competence', 'sigmoid', 'neg-sigmoid', 'gauss']:
        lng_method = 'O' if row['lng_method'] == 'opt' else 'C'
        if row['diff_score'] == 'lng_w':
            diff_score = 'Ling-CL'
        elif row['diff_score'] == 'lng':
            diff_score = lng_names[row['lng_name']]
        elif row['diff_score'] == 'loss':
            diff_score = 'Loss-CL'
        else:
            raise ValueError(row['curr'])
        lng_ids = 'Filter' if eval(row['lng_ids']) else 'Full'
        mv = 'Max' if eval(row['multiview']) else 'WAvg'
        if row['curr'] == 'sigmoid':
            curr = 'Sig'
        elif row['curr'] == 'gauss':
            curr = 'Gauss'
        elif row['curr'] == 'neg-sigmoid':
            curr = 'NegSig'
        elif row['curr'] == 'sampling':
            curr = 'Samp'
        elif row['curr'] == 'competence':
            curr = 'Comp'
        if diff_score == 'Ling-CL':
            row['curr'] = f'{diff_score} \({lng_method}\) [{curr}] [{lng_ids}]'
        elif diff_score == 'Loss-CL':
            row['curr'] = f'{diff_score} [{curr}]'
        else:
            row['curr'] = f'{diff_score}'
    if 'add_feats' in row and row['add_feats'] == 'lng':
        lng_ids = 'Filter' if eval(row['lng_ids']) else 'Full'
        row['curr'] = 'Concat [%s]'%lng_ids

    return row

def load_data():
    data = pd.concat([pd.read_csv(path) for path in args.path])
    data = data[data.run != '--']
    data.rename(lambda x: x.split('.')[1] if '.' in x else x,
            axis = 1, inplace = True)
    col_map = {
            'test_acc subset="overall" ': 'acc',
            'test_acc subset="easy" ': 'acc_easy',
            'test_acc subset="med" ': 'acc_med',
            'test_acc subset="hard" ': 'acc_hard'
            }
    data.rename(col_map, inplace=True, axis=1)
    if 'best_val_acc' in data:
        data.acc[data.acc == '-'] = data['best_val_acc']
    data = data[data.acc != '-']
    ...
    # if 'multiview' in data:
    #     data = data[
    #             (data.diff_score != 'lng_w')
    #             | ~(data['curr'].isin(['sampling', 'competence', 'sigmoid',
    #                 'neg-sigmoid', 'gauss'])) \
    #             | (data.multiview == 'True')]
    if 'sl_mode' in data:
        data = data[data.sl_mode != "avg+ent"]
    if 'lng_ids' in data:
        data.lng_ids = data.lng_ids.fillna('False')
    ...
    if 'acc_easy' in data:
        data['acc_easy'].replace('-', np.NaN, inplace=True)
        data['acc_med'].replace('-', np.NaN, inplace=True)
        data['acc_hard'].replace('-', np.NaN, inplace=True)
        ...
        data['bal_acc'] = data[['acc_easy','acc_med','acc_hard']].mean(1)
...
def vis1_4():
    ...
    bar = sns.barplot(data=subdata, x = 'curr', y = 'acc', order = order,
            hue_order = order, linewidth = 1, palette = pal, capsize = 0.1,
            ci=68
            )
    ent_ids = [idx for idx in range(len(subdata.curr.unique())) if 'Ent' in order[idx] or 'No' in order[idx] or 'Loss' in order[idx]]
    for idx in ent_ids:
...
def vis4_2():
    ...
    for i, group in enumerate(['acc_easy', 'acc_med', 'acc_hard', 'bal_acc']):
        ...
        plt.savefig('vis/bd_%s.png'%group, bbox_inches='tight')
        plt.close()
        # ax[i].set_xticks([])
    # plt.title('acc breakdown (avg)')
...
def print_for_latex_2():
    f = open('vis/latex_table.txt', 'w')
    # subdata = data[(data.noise == 0) & (data.epochs == 10) & (data.data_fraction == 1)]
    subdata = data

    means = subdata.groupby(['data', 'curr'])['acc'].mean().apply(lambda x: x*100)
    stds = subdata.groupby(['data', 'curr'])['acc'].sem().apply(lambda x: x*100)

    avg_mean = means.groupby(['curr']).mean()
    avg_std = stds.groupby(['curr']).mean()

    datasets_order = data.data.unique()
    # datasets_order = ['snli', 'anli', 'rte', 'cola']
    currs = set(subdata.curr)

    
    nocl = ['No-CL'] if 'No-CL' in currs else []
    currs = currs.difference(nocl)
    baselines = [c for c in currs if not 'Ling' in c]
    baselines.sort(key=lambda k: [x for x in k.split()])
    currs = currs.difference(baselines)
    sampling = [c for c in currs if 'Comp' in c or 'Samp' in c]
    sampling.sort(key=lambda k: [x for x in k.split()])
    weighting = [c for c in currs if 'Sig' in c or 'NegSig' in c or 'Gauss' in c]
    weighting.sort(key=lambda k: [x for x in k.split()])

    curr_groups = []
    curr_groups.append(nocl)
    curr_groups.append(baselines)
    curr_groups.append(sampling)
    curr_groups.append(weighting)
    s = '& '\
            + "".join(['\\textbf{%s} & '%d.upper() for d in datasets_order])\
            + '\\textbf{Average} '\
            + '\\\\'
    f.write(s + '\n\\hline\n')
    for currs in curr_groups:
        for curr in currs:
            s = '\\textbf{%s} & '%curr
            for dataset in datasets_order:
                if (dataset, curr) in means:
                    m, e = means[dataset, curr], stds[dataset, curr]
                    if m == means[dataset].nlargest(2).iloc[0]:
                        s += "{\\small \\textbf{%.1f}} {\\tiny $\pm$ %.2f} & "%(m, e)
                    elif m == means[dataset].nlargest(2).iloc[1]:
                        s += "{\\small \\underline{%.1f}} {\\tiny $\pm$ %.2f} & "%(m, e)
                    else:
                        s += "{\\small %.1f} {\\tiny $\pm$ %.2f} & "%(m, e)
                else:
                    s += "- & "
            avgs = avg_mean[curr], avg_std[curr]
            if avgs[0] == avg_mean.nlargest(2).iloc[0]:
                s += '{\\small \\textbf{%.1f}} {\\tiny $\pm$ %.2f}'%avgs
            elif avgs[0] == avg_mean.nlargest(2).iloc[1]:
                s += '{\\small \\underline{%.1f}} {\\tiny $\pm$ %.2f}'%avgs
            else:
                s += '{\\small %.1f} {\\tiny $\pm$ %.2f}'%avgs
            # s = s[:-3]
            s += '\\\\'
            f.write(s + '\n')
        f.write('\\hline\n')
    ...
    for dataset in datasets:
        ...
        for q in query:
            for curr in subdata_grouped.sort_values('acc', ascending = False).index:
                if curr in query:
                    continue
                s1 = subdata[subdata.curr == q].acc[:args.n]
                s2 = subdata[subdata.curr == curr].acc[:args.n]
                if len(s1) > len(s2):
                    s1 = s1[:len(s2)]
                elif len(s2) > len(s1):
                    s2 = s2[:len(s1)]
...
def vis8():
    fig = plt.figure(figsize = (8, 5.2), dpi = 300)
    subdata = data
    subdata.ent_classes = data.ent_classes.astype('int')

    bal = ['bal' if 'balanced' in x[1].data else 'full' for x in subdata.iterrows()]
    subdata['bal'] = bal

    subdata = subdata[subdata.data != 'chaosnli']

    name_map = {'alcohol_7': 'Twitter', 'snli': 'SNLI', 'cancer': 'Reddit'}
    subdata['data'] = subdata['data'].apply(lambda x: name_map[x.replace('_balanced','')])

    sns.lineplot(data = subdata, x = 'ent_classes', y = 'acc', hue = 'data',
    ...
    for dataset in data.data.unique():
        ...
        subdata.ent_classes = data.ent_classes.astype('int')
        sns.lineplot(data = subdata, x = 'ent_classes', y = 'acc',
                err_style = 'bars', ci = 68
                )
        ...
        fig.tight_layout()
        plt.savefig('vis/ent_fg_%s.pdf'%dataset)
